...
function Result({winner, replay}) {
    return (
        <div className='w-10/12 lg:w-1/2 grid medium:grid-cols-2 grid-cols-3 place-items-center'>
            <PlayButton play={winner.move} className={(winner.won === 1) ? 'shiny w-36 small:w-28' : 'w-36 small:w-28'} />
            ...
            <PlayButton play={winner.botMove} className={(winner.won === 2) ? 'shiny w-36 small:w-28' : 'w-36 small:w-28'} />
        </div>
    );
}
